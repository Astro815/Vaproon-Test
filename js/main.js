...
function tickTable() {
    ...
    for (let index = (dataUser.consultas.length - 1); index > -1; index--) {
        agTb.innerHTML += "<li id='vac_" + dataUser.consultas[index].id + "' class='" + checkTimeVacina(dataUser.consultas[index].dtNum) + "'><div class='iconVac'><span class='material-icons' style='font-size: 3em;'>event</span></div><div><h3>" + dataUser.consultas[index].name + "</h3><p><i>" + dataUser.consultas[index].data + "</i></p><br><span>" + dataUser.consultas[index].desc + "</span></div></li>";
    }
}
...
function addConsuta(name, data, desc, dataNum) {
    dataUser.consultas.push({ "name": name, "data": data, "desc": checkDesc(desc), "id": generateId(), "dtNum": dataNum });
}
...
document.querySelector("#btnConfirmAddConsulte").addEventListener("click", function() {
    // Adicionando Vacina
    const d = document;
    addConsuta(d.getElementById("nameVacAdd").value, d.getElementById("dataVacAdd").value, d.getElementById("descVacAdd").value, d.getElementById("dataVacAdd").valueAsDate);
