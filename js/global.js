...
function tickTable() {
    ...
    let model = (i) => {
        return "<a class='select' onclick='startSelect(\"" + dataUser.dataCst[dataUser.consultas[i]].id + "\")'><li id='vacItem' class='" + checkTimeVacina(dataUser.dataCst[dataUser.consultas[i]].dtNum, dataUser.dataCst[dataUser.consultas[i]].apply) + "'><div class='iconVac'><span class='material-icons' style='font-size: 3em;'>" + setIcon(checkTimeVacina(dataUser.dataCst[dataUser.consultas[i]].dtNum, dataUser.dataCst[dataUser.consultas[i]].apply)) + "</span></div><div><h3>" + dataUser.dataCst[dataUser.consultas[i]].name + "</h3><p><i>" + dataUser.dataCst[dataUser.consultas[i]].data + "</i></p><br><span>" + dataUser.dataCst[dataUser.consultas[i]].desc + "</span></div></li></a>";
    }
    agTb.innerHTML = "";
